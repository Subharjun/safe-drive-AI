...
export default function Analytics({ onNavigateToMonitor }: AnalyticsProps) {
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("⚠️ Backend API not available, checking MongoDB data...");
        // Fetch from MongoDB via new API
        const localData = await dataManager.getWellnessHistory();
        if (localData.length > 0) {
          console.log(
            `📊 Processing ${localData.length} MongoDB wellness records`
          );
          const processedData = processLocalWellnessData(localData);
          setAnalyticsData(processedData);
        } else {
          console.log("❌ No wellness data available in MongoDB");
          setAnalyticsData([]);
        }
      }
    } catch (error) {
      ...
      try {
        ...
        console.error("❌ MongoDB fallback also failed:", e);
        setAnalyticsData([]);
      }
    } finally {
      setLoading(false);
  ...
  return (
    <div className="space-y-6">
      ...
      {viewMode === "advanced" && <AdvancedAnalytics />}
                ...
                <div className="flex gap-3 justify-center">
                  ...
                  <button
                    onClick={async () => {
                      if (
                        confirm(
                          "Clear any residual analytics data from storage?"
                        )
                      ) {
                        try {
                          const cleared =
                            await dataManager.clearAnalyticsData();
                          if (cleared) {
                            alert("✅ All residual analytics data cleared!");
                            window.location.reload();
                          } else {
                            alert(
                              "⚠️ Local data cleared, but MongoDB deletion failed."
                            );
                          }
                        } catch (error) {
                          console.error("Error clearing data:", error);
                          alert(
                            "Error clearing data. Check console for details."
                          );
                        }
                      }
                    }}
